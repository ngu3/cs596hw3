...
%         g = g + A(j)*exp((-1/h)*((stars(i,1)-data(j,1)).^2 + (stars(i,2) - data(j,2)).^2));
        g = g + A(j)*(1 + data(j,1)*stars(i,1) + data(j,2)*stars(i,2)).^2;
    end
    if g > 0
        acc_pred = acc_pred + 1;
    end
...
%         g = g + A(j)*exp((-1/h)*((circles(i,1)-data(j,1)).^2 + (circles(i,2) - data(j,2)).^2));
        g = g + A(j)*(1 + data(j,1)*circles(i,1) + data(j,2)*circles(i,2)).^2;
    end
    disp(g)
    if g < 0
        acc_pred = acc_pred + 1;
...
acc = acc_pred/42;
acc_pct = acc * 100
title(sprintf('Boundary with Simpler Kernel \n with \\lambda = %2.2f, accuracy = %2.2f%%', lambda, acc_pct))
